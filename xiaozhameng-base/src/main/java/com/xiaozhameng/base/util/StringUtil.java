...
public class StringUtil {
	 ...
	 * @return
	 */
	public static double getRangeRandom(double min, double max) {
	 ...
	 * @return
	 */
	public static String generateDigest(String xml, String merchantKey) {
		// logger.info("开始生成返回报文摘要");
		String packageMac = DigestUtils.md5Hex(xml + merchantKey);
		// logger.info("结束生成返回报文摘要，摘要生成成功："+packageMac);
		return packageMac;
	}
	...
	/**
	 * 检验银行卡号，用于判断数据库中卡号是否是明文,支持12、16、18、19位卡号。 (1-30数字; 支付平台报文接口0-30位字符)
	 * 
	 * @param cardNo
	 * @return
	 */
	public static boolean regBankCardNo(String cardNo) {
		Pattern pattern = Pattern.compile("[0-9]{1,30}");
	 ...
	 * @return
	 */
	public static String tooLongSubString(String str, int len) {
		String retStr = "";
		if (StringUtil.isNullOrEmpty(str)) {
			return "";
		}
	 ...
	 * @return
	 */
	public static synchronized String nextSerialID(String merchantNo) {
		SimpleDateFormat bartDateFormat = new SimpleDateFormat("yyyyMMddHHmmss");
		merchantNo += bartDateFormat.format(System.currentTimeMillis());
		merchantNo += getFixLenthString(5);
		return merchantNo;
	}
	 ...
	 */
	public static synchronized String getFixLenthString(int strLength) {
	 ...
	 * 生成订单号-宝易互通快捷支付报文体使用
	 * <p>
	 * 订单标识1位+年月日时分秒14位+随机数8位
	 * 
	 * @return
	 */
	public static String getMerOrderId() {
		return "P" + DateTool.getYMD() + DateTool.getHMS() + generateNumber(8);
	}
	/**
	 * 获取交易流水号
	 * 
	 ...
	 * @return
	 */
	public static String getTradeFlowNo(String type) {
		return type + DateTool.getYMD() + DateTool.getHMS() + generateNumber(8);
	}
