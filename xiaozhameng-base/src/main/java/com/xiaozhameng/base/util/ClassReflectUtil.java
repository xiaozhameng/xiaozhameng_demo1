...
public class ClassReflectUtil {
	...
	/**
	 * copy 对象属性 到 另一个 对象 条件：目标对象 有 源对象的 属性
	 * 
	 * @param fromObj
	 * @param toClass
	 * @return
	...
	public static <T> T copyObjectByGetter(Object fromObj, Class<T> toClass) {
		try {
			Object toObj = toClass.newInstance();
			Map<String, Method> getMethods = new HashMap<String, Method>();
			for (Method m : fromObj.getClass().getMethods()) {
				if (m.getName().indexOf("get") != -1) {
					getMethods.put(m.getName(), m);
				}
			}
			for (Method m : toObj.getClass().getMethods()) {
				if (m.getName().indexOf("set") != -1) {
					Method method = getMethods.get(m.getName().replace("set",
							"get"));
					if (method != null) {
						try {
							Object obj = method.invoke(fromObj);
							m.invoke(toObj, obj);
						} catch (Exception e) {
							e.printStackTrace();
						}
